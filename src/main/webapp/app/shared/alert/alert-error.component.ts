...
export class AlertErrorComponent implements OnDestroy {
  ...
  constructor() {
    this.errorListener = this.eventManager.subscribe('cactusSystemApp.error', (response: EventWithContent<unknown> | string) => {
      const errorResponse = (response as EventWithContent<AlertError>).content;
      this.addErrorAlert(errorResponse.message, errorResponse.key, errorResponse.params);
    });

    this.httpErrorListener = this.eventManager.subscribe('cactusSystemApp.httpError', (response: EventWithContent<unknown> | string) => {
  ...
  private handleFieldsError(httpErrorResponse: HttpErrorResponse): void {
    ...
    for (const fieldError of fieldErrors) {
      ...
      const fieldName: string = this.translateService.instant(`cactusSystemApp.${fieldError.objectName as string}.${convertedField}`);
